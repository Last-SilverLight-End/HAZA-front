...
import { Box, Button, Center, Drawer, DrawerBody, DrawerCloseButton, DrawerContent, DrawerFooter, DrawerHeader, DrawerOverlay, Flex, IconButton, Spacer, Stack, useBreakpointValue, useColorMode, useColorModeValue, useDisclosure } from '@chakra-ui/react';
      ...
      <>
        <Stack direction={isMobile ? "column" : "row"} gap={1} align="center" justify="center">
          ...
          {loggedIn
            ?
              <UserMenu
                src="/images/profile_example.png"
                name="사람"
      ...
      <Box as="header" bg={bgColor} className="sticky top-0 z-50 backdrop-blur-lg">
        <Center width="100%" borderBottom="1px" borderColor={borderColor} paddingX={5} paddingY={1}>
          <Flex width="100%" maxWidth={maxPageWidth} height="4rem">
            ...
            <Center>
              {isDesktop
                ? FeatureSideComponent(false) // 데스크톱 모드만 렌더링
                :
                  // 모바일 모드만 렌더링
                  <>
                    <IconButton
